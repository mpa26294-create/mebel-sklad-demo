...
function materialProfileHtml(m){const a=m.attributes||{};return `<div class="audit-profile-grid"><div class="audit-card"><h5>${t('materialProfile')}</h5><div class="audit-kv"><span>${t('addedMaterial')}</span><div class="audit-kv-val">${actorNameEmailHtml(a.createdBy,a.createdByEmail)}</div></div><div class="audit-kv"><span>${t('additionDate')}</span><b>${escapeHtml(a.createdAt?auditTime(a.createdAt):'—')}</b></div><div class="audit-kv"><span>${t('lastChange')}</span><b>${escapeHtml(a.updatedAt?auditTime(a.updatedAt):'—')}</b></div></div><div class="audit-card"><h5>${t('materialHistory')}</h5>${auditListHtml('material',m.id)}</div></div>`}
...
renderHistoryStats=function(){const rows=allRows(),todayKey=auditDateOnly(new Date().toISOString()),yesterday=new Date();yesterday.setDate(yesterday.getDate()-1);const todayRows=rows.filter(r=>auditDateOnly(r.at)===todayKey).length,yesterdayRows=rows.filter(r=>auditDateOnly(r.at)===auditDateOnly(yesterday.toISOString())).length,weekRows=rows.filter(r=>Date.now()-new Date(r.at||0).getTime()<7*86400000).length,mat=rows.filter(r=>r.entity==='material').length,ord=rows.filter(r=>r.entity==='order').length,users=new Set(rows.map(r=>r.by).filter(Boolean)).size,box=document.getElementById('historyStats');if(!box)return;
  // v6.86: "-26 по сравнению со вчера" читалось двусмысленно (непонятно, это минус СЕГОДНЯ
  // или минус ВЧЕРА). Заменено на явную фразу "на N меньше/больше, чем вчера".
  const todayDelta=todayRows-yesterdayRows;
  const todayNote=todayDelta===0?t('historySameAsYesterday'):todayDelta>0?`${t('historyDeltaMorePrefix')} ${todayDelta} ${t('historyDeltaMoreSuffix')}`:`${t('historyDeltaFewerPrefix')} ${Math.abs(todayDelta)} ${t('historyDeltaFewerSuffix')}`;
  const cards=[['pulse',t('totalEvents'),rows.length,`${weekRows} ${t('historyWeek')}`],['calendar',t('today'),todayRows,todayNote],['box',t('warehouseOrders'),`${mat} / ${ord}`,t('historyUpdated')],['user',t('users'),users,t('historyActive')]];box.innerHTML=cards.map(([icon,label,value,note])=>`<div class="history-stat"><span class="history-stat-icon ${icon}">${icon==='calendar'?'▣':icon==='box'?'◇':icon==='user'?'♙':'⌁'}</span><div><small>${label}</small><b>${value}</b><em>${note}</em></div></div>`).join('')};
