...
(function(){
  ...
  function stageOpsRowHtml(kind,owner,s,index){
    ...
    const rows=ops.map((o,oi)=>`<div class="tech-op-row"><input class="input" value="${escapeHtml(o.name||'')}" onchange="${F.upd}('${id}',${index},${oi},'name',this.value)"><div class="tech-op-num" title="${escapeHtml(t('stageOpPerUnit'))}"><input class="input" type="number" min="1" step="1" value="${opPerUnit(o)}" onchange="${F.upd}('${id}',${index},${oi},'perUnit',this.value)"><span>${escapeHtml(t('stageOpPcsPerItem'))}</span></div><div class="tech-op-num" title="${escapeHtml(t('stageOpMinPerPart'))}"><input class="input" type="number" min="0" step="1" value="${Number(o.minutes||0)}" onchange="${F.upd}('${id}',${index},${oi},'minutes',this.value)"><span>${escapeHtml(t('stageOpMinPerPcs'))}</span></div><button class="iconbtn order-tech-remove" type="button" aria-label="${escapeHtml(t('deleteOperation'))}" onclick="${F.rem}('${id}',${index},${oi})">×</button></div>`).join('');
