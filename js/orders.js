...
function orderResponsibleCompactHtml(o){
  const responsible=typeof orderNextResponsibleText==='function'?orderNextResponsibleText(o):'';
  if(!responsible||responsible==='—')return '';
  const label=currentLang==='ru'?'Ответственный':currentLang==='en'?'Responsible':'Atbildīgais';
  return `<small class="order-card-responsible">${escapeHtml(label)}: ${escapeHtml(responsible)}</small>`;
}
