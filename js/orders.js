...
function orderTechnologyHtml(o){
  ...
  const hasTechData=orderSteps(o).length>0||orderMaterials(o).length>0;
  const saveAsTechBtn=hasTechData?`<button class="btn" type="button" onclick="openCreateTechnologyModal('${o.id}')">${escapeHtml(t('saveAsTechnologyBtn'))}</button>`:'';
  const updateTechBtn=o.technologyId?`<button class="btn" type="button" onclick="openTechnologySaveDialog('${o.id}')">${escapeHtml(t('updateTechnologyBtn'))}</button>`:'';
  const applyBar=typeof technologyApplyBarHtml==='function'?technologyApplyBarHtml(o):'';
  return `<div class="order-technology-screen">${applyBar}${orderTechnologyOperationsHtml(o)}${orderTechnologyMaterialsHtml(o)}${orderTechnologySummaryHtml(o)}<div class="order-tech-footer-actions"><button class="btn order-save-technology" type="button" onclick="saveTechnologyForLater('${o.id}')">${escapeHtml(t('saveAndContinueLater'))}</button>${saveAsTechBtn}${updateTechBtn}${action}</div></div>`;
}
// v7.17: операции технологии в заказе стали справочными (только чтение) — теперь их
// создают/редактируют централизованно в разделе «Технологии», а сюда попадают уже готовыми
// через блок «Применить технологию» (technologyApplyBarHtml). Список материалов заказа
// (резерв/закупка) остался редактируемым, как и раньше — это решение пользователя.
function orderTechnologyOperationsHtml(o){const qty=orderProductQty(o),steps=orderSteps(o);return `<section class="order-tech-card"><div class="order-tech-head"><div><h4>${escapeHtml(t('techOperations'))}</h4><p>${escapeHtml(t('techOperationsReadonlyHint'))}</p></div></div>${steps.length?`<div class="order-tech-table-scroll"><table class="order-tech-table"><thead><tr><th>${escapeHtml(t('operationStage'))}</th><th>${escapeHtml(t('timePerItem'))}</th><th>${escapeHtml(t('orderProductCount'))}</th><th>${escapeHtml(t('totalTime'))}</th><th>${escapeHtml(t('responsibleOptional'))}</th></tr></thead><tbody>${steps.map(s=>`<tr><td><b>${escapeHtml(typeof workshopLabel==='function'?workshopLabel(s.name||''):(s.name||''))}</b></td><td>${Number(s.minutes||0)} ${escapeHtml(t('minutesShort'))}</td><td>${qty}</td><td><b>${Number(s.minutes||0)*qty} ${escapeHtml(t('minutesShort'))}</b></td><td>${escapeHtml(s.responsible||'—')}</td></tr>`).join('')}</tbody></table></div>`:`<div class="order-tech-empty order-tech-empty-action"><b>${escapeHtml(t('techOperations'))}</b><span>${escapeHtml(t('techOperationsEmptyApplyHint'))}</span></div>`}</section>`}
