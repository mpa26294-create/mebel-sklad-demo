...
function orderResponsibilityHtml(o){
  const waiting=String(o.status)==='Ожидает технолога',technology=String(o.status)==='Технология в работе';
  if(!waiting&&!technology)return '';
  return `<div class="order-next-responsible ${technology?'active':''}"><small>${escapeHtml(t(technology?'currentResponsible':'nextResponsible'))}</small><b>👤 ${escapeHtml(t('technologistRole'))}</b></div>`;
}
...
function technologyMaterialRowHtml(o,item,index){const m=(data.materials||[]).find(x=>String(x.id)===String(item.materialId)),category=item.category||m?.category||'Поролон',unit=m?.unit||item.unit||'',per=orderItemPerUnitQty(item,o),total=calcOrderItemTotalQty(per,orderProductQty(o),unit),effective={...item,qty:total,perUnitQty:per},status=technologyMaterialStatus(o,effective),shortage=Math.max(0,Number(status.state.av.missing||0)),showOrder=per>0&&(shortage>0||orderItemPurchaseStatus(item)==='ordered'),orderQty=orderItemPurchaseQty(item,shortage);return `<div class="technology-material-row"><div class="field"><label>${escapeHtml(u42('category'))}</label><select class="select" onchange="updateTechnologyMaterial('${o.id}',${index},'category',this.value)">${ORDER_MATERIAL_CATS.map(cat=>`<option value="${cat}" ${cat===category?'selected':''}>${escapeHtml(categoryLabel(cat))}</option>`).join('')}</select></div><div class="field"><label>${escapeHtml(u42('material'))}</label><select class="select" onchange="updateTechnologyMaterial('${o.id}',${index},'materialId',this.value)">${materialOptions(category,item.materialId)}</select></div><div class="field"><label>${escapeHtml(u42('perOne'))}</label><input class="input" type="number" min="0" step="0.01" value="${Number(per||0)}" onchange="updateTechnologyMaterial('${o.id}',${index},'perUnitQty',this.value)"></div><div class="field"><label>${escapeHtml(u42('totalNeed'))}</label><div class="readonly-pill">${escapeHtml(qtyWithUnit(total,unit))}</div></div><div class="field"><label>${escapeHtml(u42('unit'))}</label><div class="readonly-pill">${escapeHtml(unit||'—')}</div></div><button class="iconbtn order-tech-remove" type="button" aria-label="${escapeHtml(t('removeMaterial'))}" onclick="removeTechnologyMaterial('${o.id}',${index})">×</button><div class="technology-material-status ${status.tone}"><span>${escapeHtml(status.label)}</span>${showOrder?`<div class="technology-order-controls"><label>${escapeHtml(t('orderedQuantity'))}</label><input class="input" id="technologyOrderQty_${o.id}_${index}" type="number" min="0.01" step="0.01" value="${Number(orderQty||0)}"><button class="btn small" type="button" onclick="markTechnologyMaterialOrdered('${o.id}',${index})">${escapeHtml(t('markAsOrdered'))}</button></div>`:''}</div></div>`}
function orderTechnologyMaterialsHtml(o){const items=orderMaterials(o);return `<section class="order-tech-card"><div class="order-tech-head"><div><h4>${escapeHtml(t('technologyMaterials'))}</h4><p>${escapeHtml(t('materialsReserveHint'))}</p></div><div class="actions"><button class="btn small" type="button" onclick="openTechnologyMaterials('${o.id}')">＋ ${escapeHtml(t('addMaterialFromStock'))}</button><button class="btn small" type="button" onclick="openTechnologyNewMaterial('${o.id}')">＋ ${escapeHtml(t('addNewMaterialToStock'))}</button></div></div><div class="technology-material-list">${items.map((item,index)=>technologyMaterialRowHtml(o,item,index)).join('')||`<div class="order-tech-empty">${escapeHtml(t('noTechnologyMaterials'))}</div>`}</div></section>`}
...
function productionMaterialsControlHtml(o){
  ...
  return `<section class="production-workflow-card production-material-control"><h4>${escapeHtml(t('productionMaterialsControl'))}</h4><div class="production-material-table-wrap"><table class="production-material-table"><thead><tr><th>${escapeHtml(t('material'))}</th><th>${escapeHtml(t('need'))}</th><th>${escapeHtml(t('prodUsed'))}</th><th>${escapeHtml(t('prodLeft'))}</th><th>${escapeHtml(t('stock'))}</th><th>${escapeHtml(t('status'))}</th></tr></thead><tbody>${items.map(i=>{const st=orderMaterialLineState(i,o.id),unit=st.av.unit||i.unit||'',used=Math.min(Number(i.qty||0),calcWorkflowProductionPercent(o)/100*Number(i.qty||0)),left=Math.max(0,Number(i.qty||0)-used),m=st.av.mat;return `<tr><td><b>${escapeHtml(m?materialTitle(m):t('deletedMaterial'))}</b></td><td>${escapeHtml(qtyWithUnit(i.qty,unit))}</td><td>${escapeHtml(qtyWithUnit(used,unit))}</td><td>${escapeHtml(qtyWithUnit(left,unit))}</td><td>${escapeHtml(qtyWithUnit(st.av.stock,unit))}</td><td><span class="production-material-status ${st.av.ok?'ok':'warn'}">${st.av.ok?'✓ '+escapeHtml(t('availableStatus')):'⚠ '+escapeHtml(t('needToPurchase'))}</span></td></tr>`}).join('')}</tbody></table></div></section>`;
}
function productionMaterialsControlHtml(o){const items=orderMaterials(o);if(!items.length)return `<section class="production-workflow-card"><h4>${escapeHtml(t('productionMaterialsControl'))}</h4><div class="order-tech-empty">${escapeHtml(t('noTechnologyMaterials'))}</div></section>`;return `<section class="production-workflow-card production-material-control"><h4>${escapeHtml(t('productionMaterialsControl'))}</h4><div class="production-material-table-wrap"><table class="production-material-table"><thead><tr><th>${escapeHtml(t('material'))}</th><th>${escapeHtml(t('need'))}</th><th>${escapeHtml(t('prodUsed'))}</th><th>${escapeHtml(t('prodLeft'))}</th><th>${escapeHtml(t('stock'))}</th><th>${escapeHtml(t('status'))}</th></tr></thead><tbody>${items.map(i=>{const st=orderMaterialLineState(i,o.id),unit=st.av.unit||i.unit||'',used=Math.min(Number(i.qty||0),calcWorkflowProductionPercent(o)/100*Number(i.qty||0)),left=Math.max(0,Number(i.qty||0)-used),m=st.av.mat,ordered=orderItemPurchaseStatus(i)==='ordered',status=st.av.ok?'✓ '+t('availableStatus'):ordered?`${t('ordered')}: ${qtyWithUnit(orderItemPurchaseQty(i,st.av.missing),unit)}`:'⚠ '+t('needToPurchase');return `<tr><td><b>${escapeHtml(m?materialTitle(m):t('deletedMaterial'))}</b></td><td>${escapeHtml(qtyWithUnit(i.qty,unit))}</td><td>${escapeHtml(qtyWithUnit(used,unit))}</td><td>${escapeHtml(qtyWithUnit(left,unit))}</td><td>${escapeHtml(qtyWithUnit(st.av.stock,unit))}</td><td><span class="production-material-status ${st.av.ok?'ok':ordered?'ordered':'warn'}">${escapeHtml(status)}</span></td></tr>`}).join('')}</tbody></table></div></section>`}
...
function markTechnologyStarted(o){if(String(o?.status)==='Ожидает технолога'){o.status='Технология в работе';auditAdd('technology_started','order',o.id,o.number,t('historyTechnologyStarted'))}}
...
async function addTechnologyMaterialFromStock(id){const o=(data.orders||[]).find(x=>String(x.id)===String(id)),materialId=document.getElementById('technologyStockMaterial')?.value,m=(data.materials||[]).find(x=>String(x.id)===String(materialId));if(!o||!m)return;if(orderMaterials(o).some(item=>String(item.materialId)===String(m.id))){toast(t('materialAlreadyInOrder'));return}o.materials=[...orderMaterials(o),{category:m.category||'',materialId:m.id,perUnitQty:0,qty:0,unit:m.unit||'',purchaseStatus:'none',purchaseQty:0,purchaseNo:''}];markTechnologyStarted(o);auditAdd('technology_material_added','order',o.id,o.number,`${t('historyMaterialAddedFromStock')}: ${materialTitle(m)}`);await persistTechnologyOrder(o);orderWorkflowSelection.set(String(o.id),1);goBackModal();refreshOrderWorkflow(o.id)}
...
async function attachCreatedTechnologyMaterial(orderId,material){const o=(data.orders||[]).find(x=>String(x.id)===String(orderId));if(!o||!material)return false;if(!orderMaterials(o).some(item=>String(item.materialId)===String(material.id)))o.materials=[...orderMaterials(o),{category:material.category||'',materialId:material.id,perUnitQty:0,qty:0,unit:material.unit||'',purchaseStatus:'none',purchaseQty:0,purchaseNo:''}];markTechnologyStarted(o);auditAdd('technology_material_created','order',o.id,o.number,`${t('historyNewMaterialAdded')}: ${materialTitle(material)}`);await persistTechnologyOrder(o);orderWorkflowSelection.set(String(o.id),1);if(typeof modalStack!=='undefined')modalStack=[];openOrderProduction(o.id);return true}
async function updateTechnologyMaterial(id,index,field,value){const o=(data.orders||[]).find(x=>String(x.id)===String(id));if(!o)return;const items=orderMaterials(o).map(item=>({...item})),item=items[index];if(!item)return;markTechnologyStarted(o);if(field==='category'){item.category=String(value||'');const first=(data.materials||[]).find(m=>m.category===item.category);item.materialId=first?.id||'';item.unit=first?.unit||''}else if(field==='materialId'){const m=(data.materials||[]).find(x=>String(x.id)===String(value));item.materialId=value;item.category=m?.category||item.category;item.unit=m?.unit||item.unit}else if(field==='perUnitQty')item.perUnitQty=Math.max(0,Number(value||0));item.qty=calcOrderItemTotalQty(item.perUnitQty,orderProductQty(o),item.unit);o.materials=items;auditAdd('technology_material_changed','order',o.id,o.number,`${t('historyTechnologyMaterialChanged')}: ${field}`);await persistTechnologyOrder(o)}
...
function orderStatusCellHtml(o,auto){
  const text=auto==='Ожидает технолога'?t('statusWaitingTechnologist'):auto==='Технология в работе'?t('statusTechnologyInProgress'):(typeof orderStatusText42==='function'?orderStatusText42(auto):auto);
  if(auto==='Не хватает материалов'){
  ...
  return `<span class="status ${orderStatusClass(auto)}">${escapeHtml(text)}</span>${typeof orderResponsibilityHtml==='function'?orderResponsibilityHtml(o):''}`;
}
...
function orderRowActions(id){
  return `<div class="order-row-action-buttons"><button class="btn open-btn" type="button" onclick="openOrderView('${id}')">${u42('open')}</button><button class="btn production-btn" type="button" title="${u42('production')}" onclick="openOrderProduction('${id}')">🛠 <span>${u42('production')}</span></button>${orderActionMenu(id)}</div>`;
}
...
async function saveManagerOrder(id=''){
  ...
  if(isNew){
    ...
    if(typeof auditAdd==='function')auditAdd('order_to_technologist','order',draft.id,draft.number,t('historyOrderSentTechnologist'),{status:draft.status,notificationMethod:method,notified:notifyEnabled});
  }
