...
function workshopAnalytics(stepName){
  ...
  if(atRisk>0)warnings.push(`${stepName}: риск не успеть к сроку — ${atRisk} ${atRisk===1?'заказ':'заказ(ов)'}`);
...
const PRODUCTION_INFO_TEXT={
  queue:['Очередь','Сколько заказов сейчас ждут выполнения операции в этом цехе — включая ещё не начатые, в работе и на паузе. Завершённые и отменённые операции в очередь не входят.'],
  active:['В работе','Сколько операций в цехе прямо сейчас реально выполняется — статус «в работе», не на паузе.'],
  plannedTime:['Плановое время','Сумма нормативного времени по всем операциям в очереди цеха: время на одно изделие по технологии × количество изделий, для каждого заказа в очереди.'],
  actualTime:['Фактическое время','Сумма реально потраченного времени по всем операциям в очереди — считается с момента старта операции; время, проведённое на паузе, не учитывается.'],
  overdue:['Просрочено','Количество заказов в очереди цеха, у которых дата сдачи уже прошла, а операция в этом цехе ещё не завершена.'],
  load:['Загрузка цеха','Отношение планового времени очереди к недельному фонду цеха — 40 часов (5 дней × 8 часов = 2400 минут). Например, 1200 / 2400 = 50%. Больше 100% значит, что цех не успевает выполнить всю текущую очередь за неделю при работе по 8 часов в день.'],
  done:['Выполнено','Сколько изделий из заказа уже подтверждено как готовые по этой операции (кнопкой «Завершить»). Может расти по частям, не обязательно всё сразу.'],
  plan:['План, мин','Нормативное время на всю партию изделий этой операции: время на 1 изделие по технологии × количество изделий в заказе.'],
  fact:['Факт, мин','Реально потраченное время с момента старта операции до сейчас (или до завершения), за вычетом времени на паузе.'],
  diff:['Разница','Факт минус План. Положительное число (красным) — операция отстаёт от норматива. Ноль или отрицательное (зелёным) — укладывается в норматив или идёт быстрее.'],
  materials:['Материалы','Показывает, хватит ли на складе материалов, привязанных к этому цеху, чтобы закрыть всю оставшуюся часть партии. «Списано материалов» — сколько комплектов уже списано под уже подтверждённые изделия, из скольких потребуется всего на заказ.'],
  sessions:['Выполнено по сменам','История подтверждений выполнения по этой операции: сколько изделий сдано за один раз, когда начата и завершена смена, кто выполнил.']
};
function showProductionInfo(key){
  const info=PRODUCTION_INFO_TEXT[key];
  if(!info)return;
  openModal(escapeHtml(info[0]),`<div class="stat-info-text">${escapeHtml(info[1])}</div>`,`<button class="btn primary" type="button" onclick="closeModal()">Понятно</button>`);
}
function infoBtn(key){return `<button type="button" class="stat-info-btn" onclick="event.stopPropagation();showProductionInfo('${key}')" aria-label="Пояснение" title="Пояснение">i</button>`}
...
function workshopOverviewRowHtml(name){
  ...
  const note=overdue?`⚠ ${stat.overdue} ${escapeHtml(t('overdue')).toLowerCase()}`:overCapacity?`⚠ ${escapeHtml(t('prodWarnOverloaded'))} (${stat.load}%)`:atRisk?`⚠ риск не успеть — ${stat.atRisk}`:nearCapacity?`⚠ ${stat.load}% ${escapeHtml(t('prodLoad')).toLowerCase()}`:'';
...
function workshopsFilterChipsHtml(){
  const items=[['all','Все'],['running','В работе'],['paused','Пауза'],['idle','Ожидает'],['overdue','Просрочено'],['atrisk','Риск срыва']];
...
function workshopQueueItemHtml(row,etaMap){
  ...
  const riskNote=(dClass!=='overdue'&&eta&&o.dueDate&&eta>o.dueDate)?`<span class="workshop-row-danger">· ⚠ риск не успеть (готово ≈ ${escapeHtml(eta)})</span>`:'';
